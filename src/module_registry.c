...
static int registry_parse(struct module_registry *reg) {
  ...
  if (!json_object_object_get_ex(root, "modules", &modules_arr) ||
    ...
    LOG_ERR("registry: missing or invalid 'modules' array in %s", reg->path);
...
static int registry_setup_watch(struct module_registry *reg) {
  ...
  if (reg->watch_fd < 0) {
    LOG_WARN("registry: inotify_add_watch failed for %s: %s", reg->watch_dir,
             strerror(errno));
    return -1;
  }
...
static struct json_object *find_module_object(struct json_object *modules,
  ...
  for (size_t i = 0; i < count; i++) {
    ...
    if (entry_vendor && entry_product && strcmp(entry_vendor, vendor_id) == 0 &&
...
int registry_register_device(struct module_registry *reg,
  ...
  if (!reg || !dev || !usb_id_is_valid(dev->vendor_id) ||
      ...
      (description &&
       strlen(description) >= sizeof(((struct module_info *)0)->description))) {
    errno = EINVAL;
    return -1;
  }
...
registry_default_attach(const struct module_registry *reg,
  ...
  return reg->defaults[cat].on_attach.has_action ? &reg->defaults[cat].on_attach
                                                 : NULL;
}
...
registry_default_detach(const struct module_registry *reg,
  ...
  return reg->defaults[cat].on_detach.has_action ? &reg->defaults[cat].on_detach
                                                 : NULL;
}
...
int registry_handle_inotify_event(struct module_registry *reg) {
  ...
  for (;;) {
    ...
    if (n < 0) {
      ...
      LOG_WARN("registry: failed to read inotify events: %s", strerror(errno));
      return -1;
    }
    if (n == 0) {
