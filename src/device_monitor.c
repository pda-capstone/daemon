    ...
    if (registry) {
        if (info) {
            if (dev->product_name[0] == '\0') {
                safe_strncpy(dev->product_name, info->name,
                             sizeof(dev->product_name));
            }
            if (info->has_sync_policy) {
                apply_sync_policy(dev, &info->sync_policy);
            } else {
                apply_sync_policy(
                    dev, registry_default_sync(registry, dev->category));
            }

            LOG_INFO("monitor: matched registry entry: %s (%s)", info->name,
                     category_to_string(info->category));
        } else {
            apply_sync_policy(dev,
                              registry_default_sync(registry, dev->category));
        }
...
int monitor_process_event(struct device_monitor *mon) {
    ...
    if (strcmp(action, "add") == 0) {
        ...
        if (tracked) {
            ...
            if (was_unclean) {
                LOG_WARN("monitor: unclean detach detected for %s (%s %s)",
                         devpath, tracked->vendor_name, tracked->product_name);
            }
        }
