...
static int do_list(DBusConnection *conn) {
    DBusMessage *reply =
        send_method_call(conn, "ListModules", DBUS_TYPE_INVALID);
    if (!reply) {
        return EXIT_FAILURE;
    }
...
static int do_info(DBusConnection *conn, const char *devpath) {
    DBusMessage *reply = send_method_call(
        conn, "GetModuleInfo", DBUS_TYPE_STRING, devpath, DBUS_TYPE_INVALID);
    if (!reply) {
        return EXIT_FAILURE;
    }
    ...
    if (!dbus_message_iter_init(reply, &iter) ||
        dbus_message_iter_get_arg_type(&iter) != DBUS_TYPE_ARRAY) {
        fprintf(stderr,
                "Error: GetModuleInfo returned invalid response type\n");
        dbus_message_unref(reply);
        return EXIT_FAILURE;
    }
        ...
        while ((msg = dbus_connection_pop_message(conn)) != NULL) {
            if (dbus_message_get_type(msg) == DBUS_MESSAGE_TYPE_SIGNAL) {
                ...
                if (strcmp(member, "ModuleAttached") == 0) {
                    ...
                    if (dbus_message_get_args(
                            msg, &sig_err, DBUS_TYPE_STRING, &devpath,
                            DBUS_TYPE_STRING, &vid, DBUS_TYPE_STRING, &pid,
                            DBUS_TYPE_STRING, &name, DBUS_TYPE_STRING,
                            &category, DBUS_TYPE_UINT32, &power,
                            DBUS_TYPE_UINT32, &speed, DBUS_TYPE_INVALID)) {
                        printf("[ATTACH] Path: %s | %s [%s:%s] | Category: %s "
                               "| MaxPower: "
                               "%u mA | Speed: %u Mbps\n",
                    ...
                    if (dbus_message_get_args(msg, &sig_err, DBUS_TYPE_STRING,
                                              &devpath, DBUS_TYPE_STRING, &name,
                                              DBUS_TYPE_BOOLEAN, &was_unclean,
                                              DBUS_TYPE_INVALID)) {
                        printf("[DETACH] Path: %s | %s | Clean detach: %s\n",
                               devpath, name,
                               was_unclean
                                   ? "NO (UNCLEAN - potential data loss)"
                                   : "YES");
                    } else {
                    ...
                    if (dbus_message_get_args(msg, &sig_err, DBUS_TYPE_UINT32,
                                              &total, DBUS_TYPE_UINT32, &count,
                                              DBUS_TYPE_INVALID)) {
                        printf("[POWER ] Total Power Draw: %u mA | Connected "
                               "Devices: %u\n",
                               total, count);
                    } else {
                    ...
                    if (dbus_message_get_args(msg, &sig_err, DBUS_TYPE_STRING,
                        ...
                        fprintf(stderr,
                                "Error parsing ModuleReadyForRemoval: %s\n",
                                sig_err.message);
                        dbus_error_free(&sig_err);
                    }
...
static void print_usage(const char *prog) {
    ...
    fprintf(stderr,
            "  list              List all currently attached modules\n");
...
int main(int argc, char *argv[]) {
    ...
    if (strcmp(cmd, "list") == 0) {
        ...
        if (argc < 3) {
            fprintf(stderr,
                    "Error: info command requires a <devpath> argument\n");
            print_usage(argv[0]);
        } else {
        ...
        if (argc < 3) {
            ...
            if (parse_register_options(argc, argv, &replace, &name, &category,
                                       &description) == 0) {
                rc = do_register(conn, argv[2], replace, name, category,
                                 description);
            }
        }
