...
static int level_to_syslog_priority(enum log_level level) {
  ...
  case LOG_LVL_VERBOSE:
    return LOG_INFO; /* syslog has no "verbose" */
  case LOG_LVL_DEBUG:
    return LOG_DEBUG;
  }
  return LOG_INFO;
...
void log_init(int use_syslog, int verbosity) {
  ...
  switch (verbosity) {
  case 0:
    g_max_level = LOG_LVL_INFO;
    break;
  case 1:
    g_max_level = LOG_LVL_VERBOSE;
    break;
  default:
    g_max_level = LOG_LVL_DEBUG;
    break;
  }
...
void log_msg(enum log_level level, const char *fmt, ...) {
  ...
  if (g_use_syslog) {
    ...
    fprintf(stderr, "%04d-%02d-%02d %02d:%02d:%02d.%03ld [%s] ",
            ...
            tm.tm_sec, ts.tv_nsec / 1000000L, level_to_prefix(level));
