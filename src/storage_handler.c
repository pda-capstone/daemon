...
static int expand_mount_point(const char *configured, const char *source,
  ...
  const char *template =
      configured && configured[0] ? configured : "/run/media/hotswapd/{device}";
  const char *device = strrchr(source, '/');
  device = device ? device + 1 : source;
  const char *token = strstr(template, "{device}");
  int written;
  if (token) {
    written = snprintf(output, output_size, "%.*s%s%s", (int)(token - template),
...
int storage_resolve_usb_parent(const char *blkdev, char *buf, size_t buflen) {
  ...
  int written = snprintf(sysblock, sizeof(sysblock), "/sys/block/%s", diskname);
...
int storage_scan_mounts(struct hs_device *dev) {
  ...
  while ((ent = getmntent(mtab)) != NULL) {
    ...
    memcpy(dev->mount_sources[dev->mount_count], ent->mnt_fsname, source_len);
  ...
  if (child == 0) {
    if (mount_options[0] != '\0') {
      execl("/bin/mount", "mount", "-o", mount_options, source, target,
            (char *)NULL);
      execl("/usr/bin/mount", "mount", "-o", mount_options, source, target,
            (char *)NULL);
    } else {
...
int storage_process_attach_once(struct hs_device *dev) {
  ...
  for (int i = 0; i < device_count; i++) {
    ...
    if (active_operations->mount_matches(devices[i], target)) {
      LOG_VERBOSE("storage: %s is already mounted at %s", devices[i], target);
    } else if (active_operations->mount_device(
                   devices[i], target, dev->on_attach_action.options) != 0) {
      LOG_DEBUG("storage: mount attempt for %s at %s failed: %s", devices[i],
...
int storage_handle_attach_timer(struct hs_device *dev) {
  ...
  uint64_t expirations;
  ssize_t count = read(dev->attach_timer_fd, &expirations, sizeof(expirations));
...
int storage_on_attach(struct hs_device *dev) {
  ...
  if (result == STORAGE_ATTACH_ERROR) {
    LOG_WARN("storage: initial attach processing failed for %s", dev->devpath);
  }
  ...
  for (int i = 0; i < dev->mount_count; i++) {
    ...
    if (active_operations->unmount_path(target, flags) != 0) {
      if (errno == EINVAL || errno == ENOENT) {
        ...
        LOG_WARN("storage: unmount of %s failed: %s", target, strerror(errno));
      }
    }
...
int storage_prepare_release(struct hs_device *dev) {
  ...
  for (int i = 0; i < dev->mount_count; i++) {
    ...
    if (!active_operations->mount_matches(source, target)) {
      LOG_WARN("storage: mount changed while preparing release: %s", target);
      failed = 1;
      continue;
    }
